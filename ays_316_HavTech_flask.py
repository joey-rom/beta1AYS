...
import shutil
import boto3
from helpers_async_s3_0_4 import (
...
from helpers_async_s3_0_4 import DASHBOARD_XLSX, CUSTOMER_EXPORT_COLUMNS
import tempfile
from datetime import datetime
...
AYS_FROM_EMAIL = os.getenv("AYS_FROM_EMAIL", "noreply@yourdomain.com")
...
def explorer_page():
    ...
    return render_template('explorer_chat_email.html')
...
def api_explorer_docs(project_id):
    ...
    def wrap(key): return f"/get?key={key}" if key else None
    ...
    for d in raw_docs:
        ...
        attachments = []
        if a.get("zip"):             attachments.append({"label": "Report ZIP",            "href": wrap(a["zip"]),             "ext": "zip"})
        if a.get("tables"):          attachments.append({"label": "Tables.xlsx",           "href": wrap(a["tables"]),          "ext": "xlsx"})
        if a.get("highlighted"):     attachments.append({"label": "Highlighted.pdf",       "href": wrap(a["highlighted"]),     "ext": "pdf"})
        if a.get("only_highlights"): attachments.append({"label": "Highlights-only.pdf",   "href": wrap(a["only_highlights"]), "ext": "pdf"})
        if a.get("email_pdf"):       attachments.append({"label": "Email Summary.pdf",     "href": wrap(a["email_pdf"]),       "ext": "pdf"})
        ...
        docs.append({
            ...
            "preview": {
                # if you upload HTML, point an artifact at it; otherwise the PDF renders in iframe
                "html": wrap(a.get("email_html")),
                "pdf":  wrap(a.get("email_pdf")),
            },
