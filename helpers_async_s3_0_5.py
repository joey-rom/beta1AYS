...
def s3_presign_get(key: str, expires: int = 3600, extra: dict | None = None) -> str:
    """
    Create a presigned GET URL. If `extra` is provided, it is merged into the
    S3 GetObject Params so you can force inline Content-Disposition/Type, e.g.:
...
def run_pipeline_to_s3(
    ...
    upload_folder: str,    # kept for signature compatibility; not used directly
    processed_folder: str,
) -> Dict[str, Any]:
    """
    Process ONE PDF and upload artifacts to S3:
      uploads/<ProjectID>/<DocFolder>/original.pdf
      results/<ProjectID>/<DocFolder>/*

    callbacks required:
      - process_pdf_file(path) -> dict  (returns {"results": {...}, "filename": <highlighted_pdf_name_in_processed_folder>})
    ...
    Returns dict with keys your routes expect, plus an HTML artifact for inline viewing.
    """
    ...
    # Scratch dir only under processed; fully removed in finally
    ...
    try:
        ...
        # Heavy processing -> writes highlighted pdf into processed_folder
        ...
        if not highlighted_name:
            raise RuntimeError("Highlighted PDF missing from results.")
        ...
        # Build small tables for summary
        results_data = results.get("results", {})
        def _extract_rows(block):
            out = []
            for r in (results_data.get(block) or []):
        ...
        total_keywords = sum(len(v or []) for v in results_data.values())
        ...
        # Email summary: HTML (for inline) + PDF (for download/zip)
        ...
        with zipfile.ZipFile(zip_local, "w") as zipf:
            ...
            # (optional) include HTML inside zip too
        ...
        # Return everything your routes and dashboard need
        return {
            "job_id": job_id,
            "project_id": project_id,
            ...
            "zip_url": zip_url,
            "ays_id": ays_id,
            "manufacturer_terms": [r["Word"] for r in manufacturer_rows if r.get("Word")],
            "recommendation": subject_summary,
            # convenience keys if your explorer maps artifacts:
            "email_pdf_key": s3_objects[os.path.basename(email_pdf_local)]["key"],
            "email_html_key": s3_objects[os.path.basename(email_html_local)]["key"],
        }

    finally:
        # Remove the highlighted PDF the processor left in the global processed folder
        _safe_unlink(highlighted_local_orig)
        # Nuke the entire per-job scratch
