# kcc_parser1.py  (DROP-IN REPLACEMENT)
...
def _write_one_sheet(
    ...
    *,
    ...
    header: dict[str, Any],
    options: list[dict[str, Any]],
    output_type: str = "shopping_list",   # ✅ NEW
) -> None:
    """
    Write ONE sheet for a single (equipment, model_code),
    containing ALL tags for that model.

    - shopping_list: table rows contain Include options per tag (current behavior)
    - all_in_one: description cell contains formatted blocks; table is empty
    """
    ...
    # Tags label (for both output types)
    tags_label = ", ".join(t for t in (tags or []) if t) if tags else ""

    # -------------------------
    # ALL-IN-ONE DESCRIPTION/NOTES
    # -------------------------
    if output_type == "all_in_one":
        # Build Base Unit vs Warranty from option category
        base_unit_lines: list[str] = []
        warranty_lines: list[str] = []
        ...
        for opt in (options or []):
            ...
            # "Category: Desc" formatting similar to your example
            line = f"{cat}: {desc}".strip(": ").strip()

            # Warranty routing: "contains warranty" (case-insensitive)
            if "warranty" in cat.lower():
                warranty_lines.append(line)
            else:
                base_unit_lines.append(line)

        # Description cell: Tags ABOVE "Base Unit"
        desc_parts: list[str] = []
        if tags_label:
            desc_parts.append(tags_label)
            desc_parts.append("")  # blank line

        desc_parts.append("Base Unit")
        desc_parts.append("")  # blank line after header
        desc_parts.extend(base_unit_lines)

        if warranty_lines:
            desc_parts.append("")  # blank line between sections
            desc_parts.append("")  # extra spacing like your example
            desc_parts.append("Warranty")
            desc_parts.append("")
            desc_parts.extend(warranty_lines)

        ws.cell(row=2, column=5, value="\n".join(desc_parts).strip())

        # Notes cell: quote/job/date/ship moved here
        notes_parts: list[str] = []
        if quote_no:
            notes_parts.append(f"Quote {quote_no}")
        if job_name:
            notes_parts.append(f"Job: {job_name}")
        if quote_date:
            notes_parts.append(f"Quote Date: {quote_date}")
        if ship_date:
            notes_parts.append(f"Ship: {ship_date}")

        ws.cell(row=2, column=6, value="\n".join(notes_parts).strip())

    # -------------------------
    # SHOPPING LIST DESCRIPTION/NOTES (existing behavior)
    # -------------------------
    else:
        desc_bits: list[str] = []
        if tags_label:
            desc_bits.append(tags_label)
        if quote_no:
            desc_bits.append(f"Quote {quote_no}")
        if job_name:
            desc_bits.append(f"Job: {job_name}")
        if quote_date:
            desc_bits.append(f"Quote Date: {quote_date}")
        if ship_date:
            desc_bits.append(f"Ship: {ship_date}")

        ws.cell(row=2, column=5, value=" | ".join(desc_bits))
        ws.cell(row=2, column=6, value="")
    ...
    # ✅ shopping_list writes option rows; all_in_one leaves table empty
    if output_type != "all_in_one":
        for tag in (tags or [""]):
            for opt in (options or []):
    ...
    for r in range(2, max(row_idx, 4)):  # ensure row 2/3 get alignment even if no rows
        for c in range(1, 21):
...
def write_kcc_template_workbook(parsed: dict[str, Any], output_type: str = "shopping_list") -> Workbook:
...
def convert_kcc_pdf_to_xlsx_bytes(
    ...
    """
    output_type:
      - "all_in_one"  -> Description = tags + Base Unit/Warranty blocks; Notes = quote meta; empty table rows
      - "shopping_list" -> current include-row table output
    """
    ...
    if out_type in ("all_in_one", "shopping_list"):
        # ✅ IMPORTANT: writer must accept output_type and pass it down
        wb = write_kcc_template_workbook(parsed, output_type=out_type)
    else:
